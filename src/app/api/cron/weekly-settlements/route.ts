...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    await new AdminNotificationService(services.supabase).create({
      ...
      title: "Weekly settlement completed",
      body: `${settlements.length} settlement action(s) processed · ${promoted} scheduled withdrawal(s) promoted · ${emailsSent} member summary email(s) sent.`,
      entityType: "cron",
      metadata: {
        priority: "success",
    ...
    logger.error("Monday weekly settlement cron failed", { message });
    try {
      const { AdminNotificationService } = await import("@/services/admin/admin-notification.service");
      await new AdminNotificationService(services.supabase).create({
        eventType: "cron.failed",
        title: "Failed CRON execution",
        body: `Weekly settlement cron failed: ${message}`,
        entityType: "cron",
        metadata: { priority: "high", cron: "weekly-settlements", error: message }
      });
    } catch {
      // non-blocking
    }
