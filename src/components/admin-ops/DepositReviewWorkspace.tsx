...
import { AlertTriangle, Check, ExternalLink, Loader2, MessageSquare, RefreshCw, X } from "lucide-react";
  ...
  async function runAction(id: string, status: string) {
    ...
    try {
      const res = await fetch(`/api/deposits/${id}`, {
        ...
        body: JSON.stringify({
          status,
          rejectionReason: status === "rejected" ? notes[id]?.trim() || "Not approved" : undefined
        })
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-xs text-zinc-500">{deposits.length} pending</p>
        ...
        >
          ...
          <label className="mt-4 block">
            ...
            <textarea
              ...
              placeholder="Rejection reason / internal note"
            />
          </label>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            >
              {busy === deposit.id ? <Loader2 size={14} className="animate-spin" /> : <Check size={14} />} Approve
            </Button>
            <Button
              size="sm"
              variant="outline"
