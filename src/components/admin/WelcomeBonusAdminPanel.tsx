...
export function WelcomeBonusAdminPanel() {
  ...
  const load = useCallback(async () => {
    try {
      const res = await fetch("/api/admin/welcome-bonus", { cache: "no-store", credentials: "same-origin" });
      const json = await res.json().catch(() => ({}));
      if (!res.ok) {
        throw new Error(typeof json.error === "string" ? json.error : "Could not load welcome bonus.");
      }
      setData(json as Snapshot);
      setLoadError("");
    } catch (err) {
      setLoadError(err instanceof Error ? err.message : "Could not load welcome bonus.");
    }
  }, []);
